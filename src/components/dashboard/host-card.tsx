...
import { CheckCircle2, ServerCrash, Trash2, XCircle } from "lucide-react";
      ...
      <CardHeader className="relative flex-row items-start justify-between">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <CardDescription className="text-sm">{host.ipAddress} - {label}</CardDescription>
            <p className="text-xs text-muted-foreground mt-1">Uptime: {formatUptime(host.createdAt)}</p>
          </div>
        </div>
      ...
      <CardContent className="flex-1">
        {host.containers.length > 0 ? (
          ...
          <div className="text-center flex flex-col items-center justify-center rounded-md border-2 border-dashed text-muted-foreground p-8 h-full">
            ...
            <p className="text-xs">This host has no containers.</p>
          </div>
        )}
      </CardContent>
    </Card>
